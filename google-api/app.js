...
{    appInit=1;
    ...
    jQuery(document).on("click", ".next", function(e) {
        "" != curLocation && curLocation.setMap(null);
            var curPage=parseInt(jQuery("#page-number").html());
            var a=curPage+1;
            if(a>=pages)
            {
            ...
            jQuery("#page-number").html(page);
            selMarker  = pager.page(page);
            $.each(markers, function(e, r) {
                r.setIcon("pin-2.png")
            }),
        ...
        jQuery(document).on("change", "#address", function(e) {
            e.preventDefault(), "" != curLocation && curLocation.setMap(null), jQuery(".selected-store .store-name").html(""), jQuery(".selected-store .store-address").html(""), jQuery(".selected-store .store-image").html(""), jQuery(".selected-store .store-description").html(""), address = jQuery("#address").val(), null === map && initializeMap(), jQuery("#page-number").html(page + 1), searchAddress(address), jQuery("#pagination").show()
        }),
        jQuery(document).on("click", "#store-form", function(e) {
            ...
            e.preventDefault(),
            "" != curLocation && curLocation.setMap(null),
                jQuery(".selected-store .store-name").html(""),
                jQuery(".selected-store .store-address").html(""),
                jQuery(".selected-store .store-image").html(""),
                jQuery(".selected-store .store-description").html(""),
                address = jQuery("#address").val(), page = 0, pageno = 1, pages = 0,
            null === map && initializeMap(),
                jQuery("#page-number").html(page + 1),
                searchAddress(address), jQuery("#pagination").show()
        }),
        jQuery(document).on("click", "#current-location", function(e) {
            e.preventDefault(), "" != curLocation && curLocation.setMap(null), navigator.geolocation && navigator.geolocation.getCurrentPosition(function(e) {
                var r = {
                ...
                marker = new google.maps.Marker({
                    ...
                    lat: r.lat,
                    lng: r.lng
                }), curLocation = marker, map.setCenter(r), map.setZoom(13), searchStoresBounds(map.getBounds().toUrlValue()), pageno = 1, page = 0,
                    jQuery(".previous").addClass("disabled"), jQuery("#page-number").html(pageno), jQuery("#pagination").show()
            }, function() {
        ...
        jQuery(document).on("click", "#reload", function(e) {
            "" != curLocation && curLocation.setMap(null), jQuery("#reload img").attr("src", "loading.gif"), jQuery("#reload img").css("opacity", "1"), jQuery(".selected-store .store-name").html(""), jQuery(".selected-store .store-address").html(""), jQuery(".selected-store .store-image").html(""), jQuery(".selected-store .store-description").html(""), searchStoresBounds(map.getBounds().toUrlValue()), pageno = 1, page = 0, jQuery(".previous").addClass("disabled"), jQuery("#page-number").html(pageno), jQuery("#reload img").attr("src", "loading.png"), jQuery("#reload img").css("opacity", "0.6"), jQuery("#pagination").show()
        }),
                    ...
                    success: function(data) {
                        ...
                        if (ship == 1) {
                            if (marker.status == '1') {
                                ...
                                item = '<h4 class="list-group-item-heading">' + t + ')' + marker.store_name + '</h4><p class="list-group-item-text"><table><tr><td style="width:175px;">' + marker.address + '<br>' + marker.distance + ' mi<br> </td><td style="padding: 0px;vertical-align: top;">' + marker.operating_hours + '</td></tr></table></p>';
                            }

                        } else if (ship == 0) {
                            if (marker.status == '1') {
                                item = '<h4 class="list-group-item-heading">' + t + ')' + marker.store_name + '</h4><p class="list-group-item-text"><table><tr><td style="width:175px;">' + marker.address + '<br>' + marker.distance + ' mi<br><strong>Live</strong> </td><td style="padding: 0px;vertical-align: top;">' + marker.operating_hours + '</td></tr><tr><td><button class="add-list" data-dismiss="modal" id="' + marker.id + '">Add to List</button></td></tr></table></p>';
                            } else if (marker.status == '2') {
                                item = '<h4 class="list-group-item-heading">' + t + ')' + marker.store_name + '</h4><p class="list-group-item-text"><table><tr><td style="width:175px;">' + marker.address + '<br>' + marker.distance + ' mi<br><strong>Pending</strong> </td><td style="padding: 0px;vertical-align: top;">' + marker.operating_hours + '</td></tr><tr><td><button class="add-list" data-dismiss="modal" id="' + marker.id + '">Add to List</button></td></tr></table></p>';
                            }
                        }
    ...
    function showStores(data, status, xhr) {
                            ...
                            success: function(data) {
                                ...
                                if (ship == 1) {
                                    if (marker.status == '1') {
                                        ...
                                        jQuery('.selected-store .store-description').html('<br><strong>Operating Hours</strong><br>' + marker.operating_hours);
                                        var content = marker.address + '<br>';
                                    }

                                } else if (ship == 0) {
                                    jQuery('.selected-store .store-description').html('<button class="add-list" data-dismiss="modal" id="' + marker.id + '">Add to List</button><br><strong>Operating Hours</strong><br>' + marker.operating_hours);
                                    var content = marker.address + '<br><button class="add-list" data-dismiss="modal" id="' + marker.id + '">Add to List</button>';
                                }
        ...
        var t = page + 1;
        pager = new Paginate(markers,inter);
        console.log(pager);
        selMarker  = pager.page(0);
        getDataList(selMarker);
    }
